...
class RegisterForm(django_auth_forms.UserCreationForm):
    error_messages = {
        'password_mismatch': u"两次密码输入不一致",
    }
    ...
    class Meta:
        model = models.Expert
        fields = ('expertname', 'keystone_username', 'email', )

    def __init__(self, request=None, *args, **kwargs):
        self.request = request
        super(RegisterForm, self).__init__(*args, **kwargs)
        self.fields['expertname'].required = True
    ...
    def _clean__keystone_username(self, data):
        if 'keystone_username' in data:
            ...
            if expert_db_handle.GetExpertInstanceByKeystoneUsername(keystone_username):
                raise forms.ValidationError(u"该账户%s已存在" % keystone_username)
    ...
    def handle(self, request, data):
        data['keystone_initial_pwd'] = data['password1']
        del data['password1']
        del data['password2']
        try:
            expert = expert_db_handle.CreateExpert(**data)
        except Exception as e:
            LOG.error(u"数据库创建Expert对象失败，data = %s, error = %s" % (str(data), str(e)))
            return False

        try:
            user = keystone_handler.user_create(expert.id, expert.keystone_username,
                                                expert.keystone_initial_pwd, expert.email)
        except Exception as e:
            LOG.error(u"Keystone创建账号失败，data = %s, expert = %s, error = %s" % (
                      str(data), str(expert), str(e)))
            expert_db_handle.DeleteExpertFromDB(expert)
            return False

        try:
            return expert_db_handle.AccountCreated(expert, user)
        except Exception as e:
            LOG.error(u"数据库关联账号失败，data = %s, expert = %s, user= %s, error = %s" % (
                      str(data), str(expert), str(user), str(e)))
            keystone_handler.user_delete(user.id)
            expert_db_handle.DeleteExpertFromDB(expert)
            return False
