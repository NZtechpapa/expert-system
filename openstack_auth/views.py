...
class RegisterView(django_auth_views.SuccessURLAllowedHostsMixin, FormView):
...
@csrf_protect
@never_cache
def register(request, template_name=None, extra_context=None, **kwargs):
    initial = {}
    if request.method == 'POST':
        userform = functional.curry(forms.RegisterForm)
    else:
        userform = functional.curry(forms.RegisterForm, initial=initial)

    res = RegisterView.as_view(
