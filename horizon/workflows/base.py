...
class ModelActionMetaclass(ActionMetaclass, forms.models.ModelFormMetaclass):
    """A Meta class for both django ModelForm and Action class
    """
    def __new__(mcs, name, bases, attrs):
        cls = forms.models.ModelFormMetaclass.__new__(mcs, name, bases, attrs)
        # Pop Meta for later processing
        opts = attrs.pop("Meta", None)
        # Process options from Meta
        cls.name = getattr(opts, "name", name)
        ...
        cls.help_text = getattr(opts, "help_text", "")
        cls.help_text_template = getattr(opts, "help_text_template", None)
        cls.maxslices = getattr(opts, "maxslices", 0)
        cls.plus_action_name = getattr(opts, "plus_action_name", '')
        cls.plus_action_hidden = False
        # if no table_format, then all fields are arranged in one line
        cls.table_format = getattr(opts, "table_format", [])
        cls.output = getattr(opts, "output", "")
        cls.model_fields = getattr(opts, "model_fields", ())
        cls.id_prefix = getattr(opts, "id_prefix", "")
        datepicker_attr = {
            'type': 'date',
        ...
        if cls.maxslices:
            if not cls.model_fields:
                cls.model_fields = cls.base_fields.keys()
            tmp_fields = OrderedDict()
            for i in range(1, 1 + cls.maxslices):
                for fname in cls.base_fields:
                    name = '%s%d' % (fname, i)
                    field = copy.deepcopy(cls.base_fields[fname])
                    if isinstance(field, DateField):
                        field.widget = DateInput(format=('%Y-%m-%d'),
                                                 attrs=datepicker_attr)
                    tmp_fields[name] = field
            for name in tmp_fields:
                cls.base_fields[name] = tmp_fields[name]
        else:
            for fname in cls.base_fields:
                if isinstance(cls.base_fields[fname], DateField):
                    cls.base_fields[fname].widget = DateInput(format=('%Y-%m-%d'),
                                                              attrs=datepicker_attr)
        return cls


@six.python_2_unicode_compatible
@six.add_metaclass(ModelActionMetaclass)
class ModelAction(Action):
    def __init__(self, request, *args, **kwargs):
        super(ModelAction, self).__init__(request,
                                          *args,
                                          **kwargs)
        if self.maxslices:
            for _src in self.model_fields:
                src = self.fields[_src]
                src.widget = forms.widgets.HiddenInput()

    def _initial_serial_field(self, name):
        readonlyInput = forms.widgets.TextInput(attrs={'readonly': 'readonly'})
        for i in range(1, 1 + self.maxslices):
            dst = self.fields['%s%d' % (name, i)]
            dst.widget = readonlyInput
            if not dst.initial:
                dst.initial = str(i)

    def clean(self):
        data = super(ModelAction, self).clean()
        if not self.maxslices:
            return data
        output = []
        for i in range(1, 1 + self.maxslices):
            not_finished = False
            ...
            for name in self.model_fields:
                ...
                if fname not in data:
                    raise ValidationError(u"序号%d: 字段%s格式错误" % (i, field.label))
                ...
                if field.label.startswith('*'):
                    if not data[fname]:
                        not_finished_area.append(field.label)
                        not_finished = True
                    if not field.initial and data[fname]:
                        not_started = False
                oneslice[name] = data[fname]
            if _start:
                today = date.today()
                if _start > today:
                    raise ValidationError(
                        u"序号%d: 起始时间‘%s’不能晚于今日‘%s’" % (i, _start.isoformat(), today.isoformat()))
                if _end and _end > today:
                    raise ValidationError(
                        u"序号%d: 结束时间‘%s’不能晚于今日‘%s’" % (i, _end.isoformat(), today.isoformat()))
                if _end and _start > _end:
                    raise ValidationError(
                        u"序号%d: 起始时间‘%s’不能晚于结束时间‘%s’" % (i, _start.isoformat(), _end.isoformat()))
            if not not_started and not_finished:
                raise ValidationError("Please fill field %s" % (",".join(not_finished_area)))
            elif not not_started:
                output.append(oneslice)
    ...
    # Only check whether one input field has initial value
    def is_initialed(self, field, fieldname):
        field_str = str(field)
        if self.fields[fieldname].label.startswith('*') and \
            'input type="text"' in field_str and \
            not 'readonly="readonly"' in field_str and \
            'value="' in field_str and\
            not 'value=""' in field_str:
            return True
        else:
            return False

    # One form are constituted by multiple slices
    # One slice contains (multiple rows with multiple fields) or (one row with multiple fields)
    # The template can construct those fields into special format
    def get_one_slice(self, curr_id, prev):
        ...
        if not self.table_format:
            ...
            for i in self.model_fields:
                ...
                if self.is_initialed(field, name):
                    has_value = True
            ...
            rows = []
            slice_id_base = None
            for row in self.table_format:
                row_id_base = None
                curr_row = {}
                elements = []
                for i in row:
                    name = "%s%d" % (i, curr_id)
                    if not slice_id_base:
                        slice_id_base = name
                    if not row_id_base:
                        row_id_base = name
                    field = self.get_field_by_name(name)
                    ...
                    if self.is_initialed(field, name):
                        has_value = True
                curr_row['id'] = "%srow_%s_%d" % (self.id_prefix, row_id_base, curr_id)
                curr_row['fields'] = elements
                rows.append(curr_row)
            oneslice['rows'] = rows
            oneslice['id'] = "%sslice_%s_%d" % (self.id_prefix, slice_id_base, curr_id)
            oneslice['actionid'] = "action_%s_%d" % (slice_id_base, curr_id)
            oneslice['actiontxt'] = self.plus_action_name if self.plus_action_name else '+'
...
@six.python_2_unicode_compatible
@six.add_metaclass(ModelActionMetaclass)
class ReadOnlyModelAction(ModelAction):
    def __init__(self, request, *args, **kwargs):
        super(ReadOnlyModelAction, self).__init__(request,
                                                  *args,
                                                  **kwargs)
        self.plus_action_hidden = True
        self.id_prefix = "detail_"
